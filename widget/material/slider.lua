...
function mat_slider:layout(_, width, height)
  ...
  table.insert(
    layout,
    base.place_widget_at(self._private.progress_bar, 0, dpi(21), width - dpi(21), height - dpi(42))
  )
  if (not self._private.read_only) then
    table.insert(
      layout,
      base.place_widget_at(self._private.slider, 0, dpi(6), width - dpi(6), height - dpi(12))
    )
  end
  return layout
end
...
local function new(args)
  ...
  ret._private.slider:connect_signal(
    "property::value", function() ret:set_value(ret._private.slider.value) end
  )
