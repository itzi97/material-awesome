...
local function list_update(w, buttons, label, data, objects)
  ...
  for i, o in ipairs(objects) do
    ...
    if cache then
      ...
      cb = clickable_container(
        wibox.container.margin(
          wibox.widget.imagebox(
            os.getenv("HOME") .. "/.config/awesome/theme/icons/tag-list/tag/close.png"
          ), 4, 4, 4, 4
        )
      )
      cb.shape = gears.shape.circle
      cbm = wibox.container.margin(cb, dpi(3), dpi(6), dpi(9), dpi(9))
      cbm:buttons(gears.table.join(awful.button({}, 1, nil, function() o.kill(o) end)))
      ...
      tt = awful.tooltip({objects = {tb}, mode = "outside", align = "bottom", delay_show = 1})

      data[o] = {ib = ib, tb = tb, bgb = bgb, tbm = tbm, ibm = ibm, tt = tt}
    end
...
local TaskList = function(s)
  return awful.widget.tasklist(
    s, awful.widget.tasklist.filter.currenttags, tasklist_buttons, {}, list_update,
